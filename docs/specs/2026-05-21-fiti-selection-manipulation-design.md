...
**Lifecycle.** When `selectedStrokeIds` changes, recompute `selectionBox` from the selected strokes' transformed-points AABB with `rotation = 0`. Gestures mutate the box *and* fan the same delta out to per-stroke transforms (kept in lock-step). The box persists at its accumulated rotation for the life of the selection.
...
The existing cursor abstraction grows from "brush-or-nil" into an enum so it can express the selection cursors without leaking AppKit into Core:

```swift
public enum SystemCursor: Equatable, Sendable {
    case arrow, openHand, closedHand
    case resizeLeftRight, resizeUpDown, resizeNWSE, resizeNESW
    case rotate
}
...
`CursorSpec` is `Sources/Core/Model/CursorSpec.swift` (a struct today — refactor to this enum). `AppController.currentCursor` stays **optional** (`CursorSpec?`): `nil` keeps the current "no managed cursor" meaning for `.inactive` mode; `.some(.brush(...))` is the pen; `.some(.system(...))` is the selection-tool cursor for the hovered region.
...
**Hover mapping (pure, tested):**
- `.rotateHandle` → `.rotate`
- `.body` → `.openHand` (`.closedHand` while a translate drag is active)
- `.outside` → `.arrow`
- `.corner(c)` → a diagonal/orthogonal cursor chosen by **screen-space angle**: take the corner's outward diagonal in box-local coords, add `box.rotation`, and bucket the result (mod 180°) to the nearest of `↔ resizeLeftRight (0°)`, `⤢ resizeNESW (45°)`, `↕ resizeUpDown (90°)`, `⤡ resizeNWSE (135°)`. At rotation 0 this yields the familiar `topLeft/bottomRight → resizeNWSE`, `topRight/bottomLeft → resizeNESW`.

During an active gesture the cursor is fixed for the gesture's duration (translate → `.closedHand`, resize → the corner's diagonal, rotate → `.rotate`).

**AppKit mapping (thin, `CursorRenderer`):** `SystemCursor → NSCursor`. Hand/arrow/left-right/up-down from public API; `resizeNWSE`/`resizeNESW` from the private window-resize selectors (`_windowResizeNorthWestSouthEastCursor` / `_windowResizeNorthEastSouthWestCursor`) guarded with a `?? .arrow` fallback; `rotate` from a curved-arrow `NSImage` drawn programmatically (no bundled asset). This is a drawing-free lookup — every decision happened in Core.

**Hover plumbing:** `AppController` gains `pointerHover(_ point:, modifiers:)`. The input view forwards `mouseMoved` (no button) to it; while the selection tool is active it computes `region` → `CursorSpec` and emits via the existing `onCursorChanged` path the brush cursor already uses.

## Gesture math

All three gestures snapshot each selected stroke's `Transform` and the `OrientedBox` at gesture start, compute from the delta (no cumulative drift), preview through `inFlightTransforms` + a live `OrientedBox`, and commit one `editor.transformStrokes` op on `pointerUp`. All math is pure `SelectionMath` functions returning the updated per-stroke transforms **and** the updated box.
...
- `Sources/Core/Selection/OrientedBox.swift` — the box value + geometry helpers (and `Point` if needed).
- `Sources/Core/Selection/SelectionMath.swift` — add `region(...)`, the `region/corner+rotation → SystemCursor` mapping, and the translate/resize/rotate math.
- `Sources/Core/Model/CursorSpec.swift` — refactor the struct to the `brush`/`system` enum; add `SystemCursor`. Update the pen path to emit `.brush(...)`.
- `Sources/Core/Control/AppController*.swift` — session state `selectionBox: OrientedBox?` (+ publisher), `pointerHover(_:modifiers:)`, region-first `pointerDown` routing, resize/rotate gesture states added to the existing `SelectionGesture` enum, transient-lifetime handling (Space-up clear with mid-gesture deferral).

**AppKit (thin, drawing-only, smoke-tested):**
- `CursorRenderer` — `SystemCursor → NSCursor` (public + private-selector diagonals + programmatic rotate image).
...
The test boundary: region classification, cursor selection, gesture math, box lifecycle, and pointer routing are all pure-Core and tested with value inputs. AppKit only does `NSCursor` lookup and pixel drawing.

## Testing strategy (red/green)

**Core — `SelectionMath` / `OrientedBox`:**
...
- cursor selection: `.rotateHandle → .rotate`, `.body → .openHand`, `.outside → .arrow`; corner bucketing — at θ=0 `.topLeft → .resizeNWSE`; rotate the box ~45° and the same corner buckets to the adjacent cursor.
- gesture math: translate (delta applied to box + each stroke); resize (uniform factor from the opposite-corner anchor, floor clamp prevents collapse, box + strokes scale around `C`); rotate (angle from center, Shift snaps to 15°, box rotation accumulates, strokes orbit + spin).
